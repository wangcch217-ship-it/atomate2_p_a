...
class PhononBSDOSDoc(StructureMetadata, extra="allow"):  # type: ignore[call-arg]
    ...
    # ===================================
        ...
        **kwargs:
        ...
        # ========== 在这里添加：提前定义 kpath 和 qpoints ==========
        # 无论是否做重整化，都需要这些变量来计算能带
        ...
        # ======================================================
        # begin to renormalize the phonon energies using SCAILD/SPOR
        if renorm_phonon:
            import shutil
            
            # 验证模式
            if pheasy_mode not in ["SCAILD", "SPOR"]:
                logger.warning(
                    f"Phonon renormalization requires SCAILD or SPOR mode, "
                    f"but pheasy_mode={pheasy_mode}. Setting to SCAILD."
                )
                pheasy_mode = "SCAILD"
            ...
            if not cal_anhar_fcs:
                raise ValueError(
                    "Phonon renormalization requires anharmonic force constants. "
                    "Please set cal_anhar_fcs=True"
                )
            
            # 设置默认 mesh
            if renorm_mesh is None:
                renorm_mesh = [20, 20, 20]
                logger.warning(f"renorm_mesh not set, using default: {renorm_mesh}")
            
            # 生成温度列表
            temperatures = list(range(renorm_temp[0], renorm_temp[1] + 1, renorm_temp[2]))
            logger.info("=" * 70)
            logger.info(f"Starting phonon renormalization using {pheasy_mode} mode")
            logger.info(f"Temperatures: {temperatures}")
            logger.info(f"Mesh: {renorm_mesh}")
            if fc_iters is not None:
                logger.info(f"Iterations: {fc_iters}")
            if fc_tol is not None:
                logger.info(f"Tolerance: {fc_tol}")
            logger.info("=" * 70)
            
            # 保存原始力常数为 0K
            if Path("FORCE_CONSTANTS").exists():
                shutil.copy("FORCE_CONSTANTS", "FORCE_CONSTANTS_0K")
                logger.info("Saved original FORCE_CONSTANTS as FORCE_CONSTANTS_0K")
            
            # 对每个温度运行 SCAILD/SPOR
            for temp in temperatures:
                logger.info("-" * 70)
                logger.info(f"Processing temperature: {temp}K")
                logger.info("-" * 70)
                ...
                pheasy_renorm_cmd = (
                    ...
                    f'-m {pheasy_mode} '
                    f'-t {temp} '
                    f'--mesh {renorm_mesh[0]} {renorm_mesh[1]} {renorm_mesh[2]} '
                    f'-f -w 4 --fix_fc2 '
                ...
                # 添加可选收敛参数
                if fc_iters is not None:
                    pheasy_renorm_cmd += f'--fc_iters {fc_iters} '
                if fc_tol is not None:
                    pheasy_renorm_cmd += f'--fc_tol {fc_tol} '
                
                # 输出为 HDF5
                pheasy_renorm_cmd += '--hdf5'
                
                logger.info(f"Command: {pheasy_renorm_cmd}")
                result = subprocess.call(pheasy_renorm_cmd, shell=True)
                
                if result != 0:
                    logger.error(f"pheasy failed for {temp}K with exit code {result}")
                    continue
                
                # 检查输出
                if not Path("FORCE_CONSTANTS").exists():
                    logger.error(f"FORCE_CONSTANTS not found after {temp}K calculation!")
                    continue
                
                # 保存此温度的力常数
                fc_temp_file = f"FORCE_CONSTANTS_{temp}K"
                shutil.copy("FORCE_CONSTANTS", fc_temp_file)
                logger.info(f"✓ Saved: {fc_temp_file}")
                
                # 读取重整化力常数并计算能带
                fc_temp = parse_FORCE_CONSTANTS(filename="FORCE_CONSTANTS")
                phonon.force_constants = fc_temp
                phonon.symmetrize_force_constants()
                
                # 保存 phonopy.yaml
                phonon.save(f"phonopy_{temp}K.yaml")
                
                # 计算能带结构
                bs_yaml_temp = f"phonon_band_structure_{temp}K.yaml"
                ...
                phonon.write_yaml_band_structure(filename=bs_yaml_temp)
                logger.info(f"✓ Saved: {bs_yaml_temp}")
                
                # 生成能带图
                bs_temp = get_ph_bs_symm_line(
                    bs_yaml_temp,
                    labels_dict=kpath_dict,
                    has_nac=born is not None
                )
                plotter_temp = PhononBSPlotter(bs=bs_temp)
                plotter_temp.save_plot(
                    filename=f"phonon_band_structure_{temp}K.pdf",
                    units=kwargs.get("units", "THz"),
                )
                logger.info(f"✓ Saved: phonon_band_structure_{temp}K.pdf")
                logger.info(f"✓ Completed {temp}K renormalization")
            
            logger.info("=" * 70)
            logger.info("All temperature renormalizations completed!")
            logger.info("=" * 70)
            
            # 恢复 0K 力常数用于后续 DOS 计算
            if Path("FORCE_CONSTANTS_0K").exists():
                shutil.copy("FORCE_CONSTANTS_0K", "FORCE_CONSTANTS")
                logger.info("Restored 0K force constants for DOS calculation")
